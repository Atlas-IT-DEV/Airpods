...
from src.services import user_services, product_services, order_product_services, promotion_services
...
def place_an_order(user_id: int, products: list[ProductsDict]):
    ...
    for order_product in list_order_products:
        ...
        if promotion.BoolCat:
            total_price += float(product.Price) * order_product.Quantity * (100 - promotion.ValueCat) * 0.01
        else:
